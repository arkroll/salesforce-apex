public class LeadUtil {
    ...
    public static Boolean mvToQueue(String oldOwner, String newOwner) {
        if(oldOwner.startsWith('005A') && (newOwner.startsWith('00GA0000002FEzJ') || newOwner.startsWith('00GA0000000YKi3'))){
            return True;
        }
        else{
            return False;
    ...
    public static Set<Lead> changeOwner(Lead lead, ID id){
        Set<Lead> s = new Set<Lead>{lead};
        return changeOwner(new Map<ID, Set<Lead>>{id => s});
    }
    ...
    public static ID campaignOwner(Lead lead){
        CampaignMember a = [Select CampaignId, Campaign_Owner__c FROM CampaignMember WHERE LeadId = :lead.Id AND route_to_rep_yn__c = 'Yes' ORDER BY SystemModstamp LIMIT 1];
        Id id = a.Campaign_Owner__c;
        return id;
    }
    
    public static String routeBy(Lead lead){
        CampaignMember a = [Select CampaignId, Member_Routing__c FROM CampaignMember WHERE LeadId = :lead.Id AND route_to_rep_yn__c = 'Yes' ORDER BY SystemModstamp DESC LIMIT 1];
